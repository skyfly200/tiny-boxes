...
contract tinyboxes is ERC721Full {
	...
	function perpetualrender(uint256 seed, uint256 colorCount, uint256 shapeCount) public returns (string memory){
    ...
    function createBoxes(uint seed) external payable returns (string memory) {
 ...
 //       require(block.timestamp < 1574711999, "ART SALE IS OVER. Tinyboxes are now only available on the secondary market.");
        ...
        if (numTokens < ARTIST_PRINTS) {
            require(msg.sender == address(0x63a9dbCe75413036B2B778E670aaBd4493aAF9F3), "Only the creator can mint the alpha token. Wait your turn FFS");
        }

        uint id = numTokens + 1;

        (uint256 colorCountDecomp, uint256 shapeCountDecomp) = getColorsandShapes(seed);

        idTocolorCount[id] = colorCountDecomp;
        idToshapeCount[id] = shapeCountDecomp;
        
        idToCreator[id] = msg.sender;
        idToRender[id] = perpetualrender(seed, colorCountDecomp, shapeCountDecomp);
        
        string memory uri = perpetualrender(seed, colorCountDecomp, shapeCountDecomp);
