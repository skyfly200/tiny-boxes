 ...
 * @updater Skyler Fly <solidity@polytopesolutions.com>
 *
 * version 1.2.0
...
library StringUtilsLib {
    ...
    function memcpy(uint _dest, uint _src, uint length) private pure {
        uint dest = _dest;
        uint src = _src;
        // Copy word-length chunks while possible
        for(uint l = length; l >= 32; l -= 32) {
            assembly {
     ...
     */
    function len(bytes32 _self) internal pure returns (uint) {
        uint ret;
        bytes32 self = _self;
        if (self == 0)
            return 0;
     ...
     */
    function count(slice memory self, slice memory needle) internal pure returns (uint counter) {
